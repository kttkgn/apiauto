...
class TestCaseService:
    ...
    async def get_test_case(self, test_case_id: int):
        """获取测试用例详情"""
        db_test_case = test_case.get(self.db, id=test_case_id)
        if not db_test_case:
    ...
    async def create_test_case(self, test_case_in: TestCaseCreate):
        """创建测试用例"""
        # 检查名称是否已存在
        db_test_case = test_case.get_by_name(self.db, name=test_case_in.name)
        if db_test_case:
            raise HTTPException(status_code=400, detail="测试用例名称已存在")
        return test_case.create(self.db, obj_in=test_case_in)
    ...
    async def delete_test_case(self, test_case_id: int):
        ...
        if not db_test_case:
            raise HTTPException(status_code=404, detail="测试用例不存在")
        return test_case.remove(self.db, id=test_case_id) 